...
theorem uniformContinuousOn_iff_edist [PseudoEMetricSpace β] {f : α → β} {s : Set α} :
...
theorem uniformContinuous_iff_edist [PseudoEMetricSpace β] {f : α → β} :
...
alias EMetric.uniformContinuousOn_iff := Metric.uniformContinuousOn_iff_edist

@[deprecated (since := "2026-01-04")]
alias EMetric.uniformContinuous_iff := Metric.uniformContinuous_iff_edist
...
/-- `EMetric.closedEBall x ε` is the set of all points `y` with `edist y x ≤ ε` -/
...
theorem nhds_eq_eball : 𝓝 x = ⨅ ε > 0, 𝓟 (eball x ε) :=
  nhds_basis_eball.eq_biInf

theorem mem_nhds_iff_eball : s ∈ 𝓝 x ↔ ∃ ε > 0, eball x ε ⊆ s :=
  nhds_basis_eball.mem_iff

theorem mem_nhdsWithin_iff_eball : s ∈ 𝓝[t] x ↔ ∃ ε > 0, eball x ε ∩ t ⊆ s :=
...
theorem tendsto_nhdsWithin_nhdsWithin_iff_edist {t : Set β} {a b} :
...
theorem tendsto_nhdsWithin_nhds_iff_edist {a b} :
    Tendsto f (𝓝[s] a) (𝓝 b) ↔
      ∀ ε > 0, ∃ δ > 0, ∀ {x : α}, x ∈ s → edist x a < δ → edist (f x) b < ε := by
  rw [← nhdsWithin_univ b, tendsto_nhdsWithin_nhdsWithin_iff_edist]
  simp only [mem_univ, true_and]

theorem tendsto_nhds_nhds_iff_edist {a b} :
    Tendsto f (𝓝 a) (𝓝 b) ↔ ∀ ε > 0, ∃ δ > 0, ∀ ⦃x⦄, edist x a < δ → edist (f x) b < ε :=
  nhds_basis_eball.tendsto_iff nhds_basis_eball

end

theorem isOpen_iff_eball : IsOpen s ↔ ∀ x ∈ s, ∃ ε > 0, eball x ε ⊆ s := by
  simp [isOpen_iff_nhds, mem_nhds_iff_eball]

@[simp] theorem isOpen_eball : IsOpen (eball x ε) :=
  isOpen_iff_eball.2 fun _ => exists_eball_subset_eball

theorem isClosed_eball_top : IsClosed (eball x ⊤) :=
  isOpen_compl_iff.1 <| isOpen_iff_eball.2 fun _y hy =>
...
theorem mem_closure_iff_edist : x ∈ closure s ↔ ∀ ε > 0, ∃ y ∈ s, edist x y < ε :=
  (mem_closure_iff_nhds_basis nhds_basis_eball).trans <| by simp only [mem_eball, edist_comm x]

theorem tendsto_nhds_iff_edist {f : Filter β} {u : β → α} {a : α} :
    Tendsto u f (𝓝 a) ↔ ∀ ε > 0, ∀ᶠ x in f, edist (u x) a < ε :=
  nhds_basis_eball.tendsto_right_iff

theorem tendsto_atTop_iff_edist [Nonempty β] [SemilatticeSup β] {u : β → α} {a : α} :
...
@[deprecated (since := "2026-01-04")] alias ball := eball
@[deprecated (since := "2026-01-04")] alias mem_ball := mem_eball
@[deprecated (since := "2026-01-04")] alias mem_ball' := mem_eball'
@[deprecated (since := "2026-01-04")] alias closedBall := closedEBall
@[deprecated (since := "2026-01-04")] alias mem_closedBall := mem_closedEBall
@[deprecated (since := "2026-01-04")] alias mem_closedBall' := mem_closedEBall'
@[deprecated (since := "2026-01-04")] alias closedBall_top := closedEBall_top
@[deprecated (since := "2026-01-04")] alias ball_subset_closedBall := eball_subset_closedEBall
@[deprecated (since := "2026-01-04")] alias pos_of_mem_ball := pos_of_mem_eball
@[deprecated (since := "2026-01-04")] alias mem_ball_self := mem_eball_self
@[deprecated (since := "2026-01-04")] alias mem_closedBall_self := mem_closedEBall_self
@[deprecated (since := "2026-01-04")] alias mem_ball_comm := mem_eball_comm
@[deprecated (since := "2026-01-04")] alias mem_closedBall_comm := mem_closedEBall_comm
@[deprecated (since := "2026-01-04")] alias ball_subset_ball := eball_subset_eball

@[deprecated (since := "2026-01-04")]
alias closedBall_subset_closedBall := closedEBall_subset_closedEBall

@[deprecated (since := "2026-01-04")] alias ball_disjoint := eball_disjoint
@[deprecated (since := "2026-01-04")] alias ball_subset := eball_subset
@[deprecated (since := "2026-01-04")] alias exists_ball_subset_ball := exists_eball_subset_eball
@[deprecated (since := "2026-01-04")] alias ball_eq_empty_iff := eball_eq_empty_iff
...
@[deprecated (since := "2026-01-04")] alias edistLtTopSetoid := edistLtTopSetoid
@[deprecated (since := "2026-01-04")] alias ball_zero := eball_zero
@[deprecated (since := "2026-01-04")] alias nhds_basis_eball := nhds_basis_eball
@[deprecated (since := "2026-01-04")] alias nhdsWithin_basis_eball := nhdsWithin_basis_eball
@[deprecated (since := "2026-01-04")] alias nhds_basis_closed_eball := nhds_basis_closedEBall

@[deprecated (since := "2026-01-04")]
alias nhdsWithin_basis_closed_eball := nhdsWithin_basis_closedEBall

@[deprecated (since := "2026-01-04")] alias nhds_eq := nhds_eq_eball
@[deprecated (since := "2026-01-04")] alias mem_nhds_iff := mem_nhds_iff_eball
@[deprecated (since := "2026-01-04")] alias mem_nhdsWithin_iff := mem_nhdsWithin_iff_eball

@[deprecated (since := "2026-01-04")] alias tendsto_nhdsWithin_nhdsWithin :=
tendsto_nhdsWithin_nhdsWithin_iff_edist

@[deprecated (since := "2026-01-04")]
alias tendsto_nhdsWithin_nhds := tendsto_nhdsWithin_nhds_iff_edist

@[deprecated (since := "2026-01-04")]
alias tendsto_nhds_nhds := tendsto_nhds_nhds_iff_edist

@[deprecated (since := "2026-01-04")] alias isOpen_iff := isOpen_iff_eball
@[deprecated (since := "2026-01-04")] alias isOpen_ball := isOpen_eball
@[deprecated (since := "2026-01-04")] alias isClosed_ball_top := isClosed_eball_top
@[deprecated (since := "2026-01-04")] alias ball_mem_nhds := eball_mem_nhds
@[deprecated (since := "2026-01-04")] alias closedBall_mem_nhds := closedEBall_mem_nhds
@[deprecated (since := "2026-01-04")] alias ball_prod_same := eball_prod_same
@[deprecated (since := "2026-01-04")] alias closedBall_prod_same := closedEBall_prod_same
@[deprecated (since := "2026-01-04")] alias mem_closure_iff := mem_closure_iff_edist
@[deprecated (since := "2026-01-04")] alias tendsto_nhds := tendsto_nhds_iff_edist
@[deprecated (since := "2026-01-04")] alias tendsto_atTop := tendsto_atTop_iff_edist
