...
theorem isUniformInducing_iff_edist [PseudoEMetricSpace β] {f : α → β} :
...
theorem isUniformEmbedding_iff_edist [PseudoEMetricSpace β] {f : α → β} :
    IsUniformEmbedding f ↔ Function.Injective f ∧ UniformContinuous f ∧
      ∀ δ > 0, ∃ ε > 0, ∀ {a b : α}, edist (f a) (f b) < ε → edist a b < δ :=
  (isUniformEmbedding_iff _).trans <| and_comm.trans <| Iff.rfl.and isUniformInducing_iff_edist
...
TODO: generalize? -/
theorem controlled_edist_of_isUniformEmbedding [PseudoEMetricSpace β] {f : α → β}
    (h : IsUniformEmbedding f) :
    (∀ ε > 0, ∃ δ > 0, ∀ {a b : α}, edist a b < δ → edist (f a) (f b) < ε) ∧
      ∀ δ > 0, ∃ ε > 0, ∀ {a b : α}, edist (f a) (f b) < ε → edist a b < δ :=
  ⟨uniformContinuous_iff_edist.1 h.uniformContinuous, (isUniformEmbedding_iff_edist.1 h).2.2⟩

/-- ε-δ characterization of Cauchy sequences on pseudoemetric spaces -/
theorem cauchy_iff_edist {f : Filter α} :
...
theorem complete_of_convergent_controlled_sequences_edist (B : ℕ → ℝ≥0∞) (hB : ∀ n, 0 < B n)
...
theorem tendstoLocallyUniformlyOn_iff_edist {ι : Type*} [TopologicalSpace β] {F : ι → β → α}
...
theorem tendstoUniformlyOn_iff_edist {ι : Type*} {F : ι → β → α} {f : β → α} {p : Filter ι}
    {s : Set β} :
...
theorem tendstoLocallyUniformly_iff_edist {ι : Type*} [TopologicalSpace β] {F : ι → β → α}
    {f : β → α} {p : Filter ι} :
    TendstoLocallyUniformly F f p ↔
      ∀ ε > 0, ∀ x : β, ∃ t ∈ 𝓝 x, ∀ᶠ n in p, ∀ y ∈ t, edist (f y) (F n y) < ε := by
  simp only [← tendstoLocallyUniformlyOn_univ, tendstoLocallyUniformlyOn_iff_edist, mem_univ,
    forall_const, nhdsWithin_univ]

/-- Expressing uniform convergence using `edist`. -/
theorem tendstoUniformly_iff_edist {ι : Type*} {F : ι → β → α} {f : β → α} {p : Filter ι} :
    TendstoUniformly F f p ↔ ∀ ε > 0, ∀ᶠ n in p, ∀ x, edist (f x) (F n x) < ε := by
  simp only [← tendstoUniformlyOn_univ, tendstoUniformlyOn_iff_edist, mem_univ, forall_const]
...
alias isUniformInducing_iff := isUniformInducing_iff_edist

@[deprecated (since := "2026-01-04")]
alias isUniformEmbedding_iff := isUniformEmbedding_iff_edist

@[deprecated (since := "2026-01-04")]
alias controlled_of_isUniformEmbedding := controlled_edist_of_isUniformEmbedding

@[deprecated (since := "2026-01-04")]
alias cauchy_iff := cauchy_iff_edist

@[deprecated (since := "2026-01-04")]
alias complete_of_convergent_controlled_sequences :=
  complete_of_convergent_controlled_sequences_edist

@[deprecated (since := "2026-01-04")]
alias complete_of_cauchySeq_tendsto := Metric.complete_of_cauchySeq_tendsto

@[deprecated (since := "2026-01-04")]
alias tendstoLocallyUniformlyOn_iff := tendstoLocallyUniformlyOn_iff_edist

@[deprecated (since := "2026-01-04")]
alias tendstoUniformlyOn_iff := tendstoUniformlyOn_iff_edist

@[deprecated (since := "2026-01-04")]
alias tendstoLocallyUniformly_iff := tendstoLocallyUniformly_iff_edist

@[deprecated (since := "2026-01-04")]
alias tendstoUniformly_iff := tendstoUniformly_iff_edist
...
theorem inseparable_iff_edist : Inseparable x y ↔ edist x y = 0 := by
  simp [inseparable_iff_mem_closure, mem_closure_iff_edist, edist_comm, forall_gt_iff_le]

alias ⟨_root_.Inseparable.edist_eq_zero, _⟩ := inseparable_iff_edist

-- see Note [nolint_ge]
/-- In a pseudoemetric space, Cauchy sequences are characterized by the fact that, eventually,
the pseudoedistance between its elements is arbitrarily small -/
theorem cauchySeq_iff_edist [Nonempty β] [SemilatticeSup β] {u : β → α} :
...
`ℝ≥0` upper bounds. -/
theorem cauchySeq_iff_edist_nnreal [Nonempty β] [SemilatticeSup β] {u : β → α} :
    CauchySeq u ↔ ∀ ε : ℝ≥0, 0 < ε → ∃ N, ∀ n, N ≤ n → edist (u n) (u N) < ε :=
  uniformity_basis_edist_nnreal.cauchySeq_iff'

theorem totallyBounded_iff_eball {s : Set α} :
...
theorem subset_countable_closure_of_almost_dense_set (s : Set α)
    (hs : ∀ ε > 0, ∃ t : Set α, t.Countable ∧ s ⊆ ⋃ x ∈ t, closedEBall x ε) :
...
theorem subset_countable_closure_of_compact {s : Set α} (hs : IsCompact s) :
  ...
  rcases totallyBounded_iff_eball'.1 hs.totallyBounded ε hε with ⟨t, -, htf, hst⟩
...
theorem secondCountable_of_almost_dense_set_closedEBall
...
instance (priority := 100) EMetricSpace.instT0Space : T0Space γ where
  t0 _ _ h := eq_of_edist_eq_zero <| Metric.inseparable_iff_edist.1 h

/-- A map between emetric spaces is a uniform embedding if and only if the edistance between `f x`
and `f y` is controlled in terms of the distance between `x` and `y` and conversely. -/
theorem Metric.isUniformEmbedding_iff_edist' [PseudoEMetricSpace β] {f : γ → β} :
    IsUniformEmbedding f ↔
      (∀ ε > 0, ∃ δ > 0, ∀ {a b : γ}, edist a b < δ → edist (f a) (f b) < ε) ∧
        ∀ δ > 0, ∃ ε > 0, ∀ {a b : γ}, edist (f a) (f b) < ε → edist a b < δ := by
  rw [isUniformEmbedding_iff_isUniformInducing, isUniformInducing_iff_edist,
...
abbrev EMetricSpace.ofT0PseudoEMetricSpace (α : Type*) [PseudoEMetricSpace α] [T0Space α] :
  ...
  { ‹PseudoEMetricSpace α› with
    eq_of_edist_eq_zero := fun h => (Metric.inseparable_iff_edist.2 h).eq }
...
instance [PseudoEMetricSpace X] : EDist (SeparationQuotient X) where
  edist := SeparationQuotient.lift₂ edist fun _ _ _ _ hx hy =>
    edist_congr (Metric.inseparable_iff_edist.1 hx) (Metric.inseparable_iff_edist.1 hy)
